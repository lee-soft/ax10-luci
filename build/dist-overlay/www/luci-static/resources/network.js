...
ports.sort(function(a,b){if(a.role!=b.role)
return(a.role<b.role)?-1:1;return(a.index-b.index);});for(var i=0,port;(port=ports[i])!=null;i++){if(port.role!=role){role=port.role;pnum=1;}
if(role=='cpu')
...
function networkSort(a,b){return strcmp(a.getName(),b.getName());}
function deviceSort(a,b){var typeWeigth={wifi:2,alias:3},weightA=typeWeigth[a.getType()]||1,weightB=typeWeigth[b.getType()]||1;if(weightA!=weightB)
return weightA-weightB;return strcmp(a.getName(),b.getName());}
...
rv.push(this.lookupWifiNetwork(wifiIfaces[i]['.name']));rv.sort(function(a,b){return strcmp(a.getID(),b.getID());});return rv;},this));},addWifiNetwork:function(options){return initNetworkState().then(L.bind(function(){if(options==null||typeof(options)!='object'||typeof(options.device)!='string')
...
rv.sort(function(a,b){if(a.metric!=b.metric)
return(a.metric-b.metric);return strcmp(a.interface,b.interface);});return rv;},this));},getStatusByAddress:function(addr){return initNetworkState().then(L.bind(function(){var rv=[];for(var i=0;i<_state.ifaces.length;i++){if(Array.isArray(_state.ifaces[i]['ipv4-address']))
...
return rv.sort(function(a,b){return strcmp(a[0],b[0]);});}});Protocol=baseclass.extend({__init__:function(name){this.sid=name;},_get:function(opt){var val=uci.get('network',this.sid,opt);if(Array.isArray(val))
